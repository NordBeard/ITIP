...
public class Main {
    ...
    public static String fractions(String number) {
        ...
        denominators[0] = (int) Math.pow(10, numArray[1].length());
        denominators[1] = ((int) Math.pow(10, numArray[2].length()) - 1) * denominators[0];
        if (numArray[1].isEmpty()) {
            numArray[1] = "0";
        }
        int numerator = Integer.parseInt(numArray[0]) * denominators[1] +
                Integer.parseInt(numArray[1]) * (denominators[1] / denominators[0]) +
                Integer.parseInt(numArray[2]);
        int gcd = FirstPractice.gcd(numerator, denominators[1]);
        String strNumerator = Integer.toString(numerator / gcd);
        String strDenominator = Integer.toString(denominators[1] / gcd);
